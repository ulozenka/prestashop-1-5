...
if($version == 160 && '$targetClass' != 'carrier_infos') {
 var mycell= $(test[$num]).find(".delivery_option_logo").next(); 
$(mycell).append( "<span id='ulozenka'></span> &nbsp; <strong>Vyberte dodací místo: </strong><select name='ulozenka' style='border:1px solid #CCCCCC;margin-left:10px' id='selulozenka' onChange='selPobocka($num);'>$pobockstr</select>    $detail" )
}else {
$(test[$num]).append( "<span id='ulozenka'></span> &nbsp; <strong>Vyberte dodací místo: </strong><select name='ulozenka' style='border:1px solid #CCCCCC;margin-left:10px' id='selulozenka' onChange='selPobocka($num);'>$pobockstr</select>    $detail" )
...
ULOZENKA;
    ...
    protected function getCarrierOptionNum($params) {
        $num = 0;
